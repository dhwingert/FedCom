...
public class DragNDropListView extends ListView {
	...
	int mDragPointOffsetY; // Used to adjust drag view location
    int mDragPointOffsetX; // Used to adjust drag view location
	int mDragHandler = 0;
		...
		switch (action) {
			case MotionEvent.ACTION_DOWN:
				...
				if (mStartPosition != INVALID_POSITION) {
                    ...
                    mDragPointOffsetY = y - getChildAt(childPosition).getTop();
                    mDragPointOffsetY -= ((int)ev.getRawY()) - y;

                    mDragPointOffsetX = ((int)ev.getRawX());
	...
	private void startDrag(int childIndex, int y) {
        ...
        // Draw a rectangle around the item being dragged
    ...
    // If a row reports that it was clicked, tell the listener
    public void rowClicked(View view, BaseInfo baseInfo) {
        mDragNDropListener.onRowClicked(view, baseInfo);
    }
}
